...
fn run(args: Args) -> anyhow::Result<()> {
    ...
    println!("Running with negative prompt \"{negative_prompt}\".");
    let tokens = tokenizer.encode(&prompt)?;
    let tokens: Vec<i64> = tokens.into_iter().map(|x| x as i64).collect();
    let tokens = Tensor::from_slice(&tokens).view((1, -1)).to(clip_device);
    let uncond_tokens = tokenizer.encode(&negative_prompt)?;
    ...
    let base_path = "./results/";
    let now = Utc::now();
    let datetime_str = format!("{}_{}_{}_{}-{}", now.year(), now.month(), now.day(), now.hour(), now.minute());
    let mut folder_name = format!("{}({})", final_image, datetime_str);
    let mut full_path = Path::new(base_path).join(folder_name.clone());
    let mut counter = 1;
    while full_path.exists() {
        folder_name = format!("{}_{}", folder_name, counter);
        full_path = Path::new(base_path).join(&folder_name);
        counter += 1;
    }
    match fs::create_dir(&full_path) {
        Ok(_) => println!("Folder {:?} created successfully!", full_path),
